...
`volatile`是轻量级的线程操作可见方式,并非同步方式,如果是多写场景,一定会产生线程安全问题.如果是一写多读的并发场景,使用`volatile`修饰变量则非常合适;

#### 信号量同步

信号量同步,是指不同的线程之间,通过传递同步信号量来协调线程执行的先后次序.
+ 时间维度:`CountDownLatch`
+ 信号维度:`Semaphore`
+ 基于同步到达某个点的信号量触发机制:`CyclicBarrier`