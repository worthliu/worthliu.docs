...
`JVM`内存布局规定了Java在运行过程中内存申请,分配,管理的策略,保证了JVM的高效稳定运行;
...
>1. `Heap`(堆区)
 ...
 + 堆分成两大块:`新生代`和`老年代`:
  ...
  + 出错时的堆内信息对解决问题非常有帮助,`JVM`设置运行参数`-XX:+HeapDumpOnOutOfMemoryError`,让`JVM`遇到OOM异常时能输出堆内信息;

>2. `Metaspace`(元空间)
+ `JDK8`版本中,元空间的前身`Perm区`已经被淘汰.
+ `JDK7`及之前的版本中,只有Hotspot才有`Perm区`,译为永久代,**它在启动时间固定大小,很难进行调优,并且`FGC`时会移动类元信息;**
